...
router = APIRouter(prefix="/analysis", tags=["Analyst workspace"])
...
def get_analysis_workspace(
    ...
    anomaly_record = get_latest_anomaly_run(
        database,
        analysis_id=analysis.id,
    )

    report_record = get_latest_investigation_report(
        database,
        analysis_id=analysis.id,
    )
    ...
    if anomaly_record is not None:
        anomaly_response = _build_anomaly_response(anomaly_record)

    report_response = None

    if report_record is not None:
        report_response = _build_report_response(report_record)
    ...
    return AnalysisWorkspaceResponse(
        ...
        latest_anomaly_run=(anomaly_response),
        latest_investigation_report=(report_response),
    )
