...
def get_cached_environment(
    session: Session, *, provider_name: str, indicator: Indicator
...
def save_enrichment(
    ...
    *,
    ...
    record.result_json = reputation.model_dump(mode="json")
...
def enrich_indicators(
    ...
    *,
    ...
    for indicator in indicators:
        ...
        if indicator.type == IndicatorType.IP_ADDRESS and not is_public_ip(
            indicator.value
        ):
            items.append(
                ThreatIntelItem(
                    ...
                    provider=(provider.provider_name),
                    reason=(
                        "Non-public IP addresses are " "not sent to external providers."
                    ),
                )
            )
        ...
        if cached is not None:
            reputation = IPReputation.model_validate(cached.result_json)
