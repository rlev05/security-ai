...
def build_auth_log(
    *,
    ...
    for index in range(anomalous_events):
...
def test_user_can_create_anomaly_run(
    ...
    assert result["analysed_events"] == 68
...
def test_history_lists_multiple_runs(
    ...
    first = client.post(
        (f"/analysis/{analysis_id}" "/anomalies" "?contamination=0.05"),
        headers=headers,
    )
    ...
    response = client.get(
        (f"/analysis/{analysis_id}" "/anomalies/history"),
        headers=headers,
    )
...
def test_anomaly_endpoints_require_authentication(
    ...
    run_response = client.post(f"/analysis/{analysis_id}" "/anomalies")

    latest_response = client.get(f"/analysis/{analysis_id}" "/anomalies")
