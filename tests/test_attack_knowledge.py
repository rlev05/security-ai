...
def test_extracts_detected_attack_ids() -> None:
    ...
    technique_ids = extract_detected_technique_ids(result)
...
def test_retrieves_known_technique() -> None:
    ...
    technique = repository.get_technique("T1110.001")
...
def test_builds_grounding_from_detection() -> None:
    ...
    context = repository.build_grounding_context(
        {"incidents": [{"alerts": [{"mitre_technique_id": ("T1110.003")}]}]}
    )

    assert len(context.techniques) == 1

    assert context.techniques[0].technique_id == "T1110.003"
...
def test_tracks_unknown_techniques() -> None:
    ...
    context = repository.build_grounding_context(
        {"alerts": [{"mitre_technique_id": ("T9999.999")}]}
    )

    assert context.techniques == []

    assert context.unresolved_technique_ids == ["T9999.999"]
