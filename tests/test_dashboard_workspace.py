...
PASSWORD = "Dashboard-Workspace-Password-123!"
...
def test_workspace_requires_dashboard_authentication(
    ...
    response = client.get(
        ("/dashboard/analysis/" "00000000-0000-0000-0000-000000000001"),
        follow_redirects=False,
    )
...
def test_workspace_displays_analysis_evidence(
    ...
    response = client.get(f"/dashboard/analysis/" f"{analysis_id}")

    assert response.status_code == 200, response.text

    assert "Deterministic Detections" in response.text

    assert "Parsed Security Events" in response.text

    assert "ML Anomaly Detection" in response.text

    assert "Investigation Report" in response.text

    assert "203.0.113.250" in response.text

    assert "No AI investigation report" in response.text
...
def test_workspace_displays_latest_ml_anomaly_run(
    ...
    assert "Model run" in response.text
...
def test_workspace_does_not_expose_another_users_analysis(
    ...
    owner_username, owner_token = register_user_and_get_token(client)
    ...
    other_username, _ = register_user_and_get_token(client)
